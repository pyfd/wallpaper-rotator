...
A small local status page — current wallpaper, pool size/disk, per-source
download tallies, recent activity, and config. Served on demand:

```bash
wallpaper-web                       # -> http://127.0.0.1:8787  (Ctrl-C to stop)
```

It binds to `127.0.0.1` only (not exposed on the network), needs `python3`, and
the page auto-refreshes every 30s. Cron regenerates it each tick, so it stays
current while the server runs. The page itself is a static file
(`~/.local/state/wallpaper-rotator/web/index.html`) regenerated by
`gen-status.sh` — no always-on process. Change the port via `WEB_PORT` in
`install.sh`.
...
│   ├── gen-status.sh                        # builds the status page (templated)
│   └── wallpaper-web.sh                     # serves the status page on 127.0.0.1 (templated)
...
`@@LOG@@`, `@@WEBDIR@@`, `@@PORT@@`, `@@RES@@`, `@@SOURCES@@`, `@@LOCALDIR@@`,
`@@TARGET@@`, `@@RESOLUTION@@`, `@@USER@@` placeholders that `install.sh`
substitutes per-machine.
